...
public class StaticAnalysisLabel implements ASTVisitor{
	
	int counter=0;
	List<Integer> blockPattern = new ArrayList<>();
	//ArrayList<StatLabelRep> statLabelList = new ArrayList<>();
	...
	public Object visitBlock(Block block, Object arg) throws Exception {
		...
		//List<Integer> li = blockPattern;
		
		for(int i=0;i<stats.size();i++) {
			if(stats.get(i) instanceof StatLabel) {
				StatLabelRep slr = new StatLabelRep((StatLabel) stats.get(i));
				slr.blockPattern.addAll(blockPattern);
				//statLabelList.add(slr);
				((List<StatLabelRep>) arg).add(slr);
				//System.out.println("yyyyyy"+statLabelList);
			}else if(stats.get(i) instanceof StatDo || stats.get(i) instanceof StatWhile || stats.get(i) instanceof StatRepeat || stats.get(i) instanceof StatIf) {
	...
	public Object visitChunk(Chunk chunk, Object arg) throws Exception {
		Block b = chunk.block;
		b.visit(this, arg);
		//System.out.println("zzzzz"+statLabelList);
		return null;
	}
