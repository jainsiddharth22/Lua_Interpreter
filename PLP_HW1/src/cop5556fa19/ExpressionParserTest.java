...
class ExpressionParserTest {
	...
	void tests2() throws Exception{
		...
		Exp expected = Expressions.makeBinary(
				Expressions.makeBinary(2,OP_PLUS,3)
				, OP_PLUS
	...
	void tests3() throws Exception{
		...
		Exp expected = Expressions.makeExpUnary(
				 OP_MINUS,
				Expressions.makeBinary(4,OP_MINUS,2));
		show("expected="+expected);
		assertEquals(expected,e);
	}
	...
	@Test
	void tests5() throws Exception{
		String input = "{[x]=4,y=2}";
		Exp e = parseAndShow(input);
	}
	
	@Test
	void tests72() throws Exception{
		String input = "not #~1";
		Exp e = parseAndShow(input);
	...
	@Test
	void tests8() throws Exception{
		String input = "{[x]=4,,y=2;x,}";
		assertThrows(SyntaxException.class, () -> {
			Exp e = parseAndShow(input);
			});	
	}
