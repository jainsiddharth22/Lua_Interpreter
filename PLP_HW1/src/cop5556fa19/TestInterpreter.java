...
//		static final boolean doPrint = false;
		...
		@Test
		void run22232() throws Exception{
			String input = "i=1 do do return 35 end i=1 i=2 return 55 end return 45";
			show(input);
			List<LuaValue> ret = interpret(input);
			show(ret);
			LuaValue[] vals = {new LuaInt(35)};
		...
		@Test
		void run222332() throws Exception{
			String input = "return 2==1+1";
			show(input);
			List<LuaValue> ret = interpret(input);
			show(ret);
			LuaValue[] vals = {new LuaBoolean(true)};
			List<LuaValue> expected = Arrays.asList(vals);
			assertEquals(expected, ret);
		}
		...
		@Test
		void break023() throws Exception {
			String input = "i=0 while i<10 do if i==2 then break end i=i+1 end return i";
			show(input);
			List<LuaValue> ret = interpret(input);
			show(ret);		
			List<LuaValue> expected = new ArrayList<>();
...
//			expected.add(LuaInt.valueOf(2));			
		...
		void testSetField0() throws Exception{
			String input = "a = 4; t={}; t[a]=45 return t[a]";
			show(input);
			List<LuaValue> ret = interpret(input);
			show(ret);	
			List<LuaValue> expectedList = new ArrayList<>();
//			LuaTable expected = new LuaTable();
//			expectedList.add(expected);
//			expected.put(new LuaInt(4), new LuaInt(45));
			expectedList.add(new LuaInt(45));
			assertEquals(expectedList,ret);
		}
